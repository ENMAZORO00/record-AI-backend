...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (existingAdmin) {
      return res.status(409).json({ error: 'You are already an admin of a company' })
    }
...
router.post('/invites', async (req, res, next) => {
  try {
    ...
    if (!admin.companyId || admin.companyRole !== 'admin') {
      return res.status(403).json({ error: 'Only company admin can add members' })
    }
    ...
    if (existingUser?.companyId === admin.companyId) {
      return res.status(409).json({ error: 'This user is already in your company' })
    }
    if (existingUser?.companyId) {
      return res.status(409).json({ error: 'This user already belongs to another company' })
    }
    ...
    setImmediate(() => {
      sendEmployeeCredentialsEmail(email, company.name, admin.name, email, plainPassword).catch((err) => {
        console.error('Employee credentials email failed:', err)
      })
    })

    res.status(201).json({ message: 'Employee added. Login details sent to their email.', email })
  } catch (err) {
    next(err)
  }
})
...
router.post('/join', async (req, res, next) => {
  try {
    const inviteToken = (req.body?.inviteToken ?? req.body?.token ?? '').toString().trim()
    if (!inviteToken) {
      return res.status(400).json({ error: 'Invite token is required' })
    }
    const user = req.user
    ...
    if (new Date() > invite.expiresAt) {
      await prisma.companyInvite.delete({ where: { id: invite.id } }).catch(() => {})
      return res.status(400).json({ error: 'Invite has expired' })
    }
